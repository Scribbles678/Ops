...
export default defineEventHandler(async (event) => {
  ...
  const { employee_id, job_function_id, am_job_function_id, pm_job_function_id, is_required = false, priority = 0, notes } = body
  ...
  const result = await query(
    `INSERT INTO preferred_assignments (employee_id, job_function_id, am_job_function_id, pm_job_function_id, is_required, priority, notes, team_id)
     ...
     RETURNING *`,
    [employee_id, job_function_id, am_job_function_id ?? null, pm_job_function_id ?? null, is_required, priority, notes ?? null, teamId ?? null]
  )
